"""
Simple autoregressive evaluation on sampled windows from data/test_combined.txt.

This script keeps the same GT-control decoding protocol used in training
validation: it autoregressively predicts score triplets while inserting the
ground-truth performance control triplets between predictions.

Outputs:
  - aggregate_stats.json
  - per_sequence_stats.json
  - sampled_indices.json
  - summary.txt
  - sequence_<idx>/input_performance.mid
  - sequence_<idx>/ground_truth_score.mid
  - sequence_<idx>/output_score.mid
"""
...
OUTPUT_BASE = "test_combined_autoregressive_results"
DEFAULT_NUM_EXAMPLES = 25
DEFAULT_RANDOM_SEED = 41
DEFAULT_CONFIG_SOURCE = "hf-ckpt-3500/checkpoint-3500"
K_PREFIX = 33
...
def guess_default_checkpoint() -> str:
    ...
    for path in [
        ...
        if weight_path.exists():
            candidates.append((weight_path.stat().st_mtime, str(path)))
    if not candidates:
...
def write_summary(
    ...
    summary_lines = [
        "Test Combined Autoregressive Evaluation",
        "=" * 60,
...
def main():
    parser = argparse.ArgumentParser(
        description="Autoregressive score evaluation on sampled test_combined windows"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=TEST_FILE,
        help="Path to the test_combined token file",
    )
    parser.add_argument(
        "--num-examples",
        type=int,
    ...
    print("\n" + "=" * 60)
    print("AUTOREGRESSIVE EVALUATION COMPLETE")
    print("=" * 60)
