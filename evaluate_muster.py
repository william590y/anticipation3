...
ALTERNATING_START = K_PREFIX * 2 * EVENT_SIZE  # = 198
...
def autoregressive_generate_full_piece(model, full_gt_tokens, score_start_idx, device,
    ...
    `full_gt_tokens` may be arbitrarily longer than PACKED_SEQUENCE_LENGTH. The
    fixed control/rest prefix (first `score_start_idx` tokens) is always kept
    in the context.
    Whenever `context` grows to PACKED_SEQUENCE_LENGTH, the first half of the
    alternating section is dropped, aligned to 6-token (score+control pair)
    boundaries.
